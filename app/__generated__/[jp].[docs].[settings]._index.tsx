...
<Image
src={imagePath + "/Settings-ja.webp?raw=true"}
loading={"lazy"}
alt={"設定"}
className={`w-image c13rvnfs c12e0v0g c1eswi8f`} />
<MarkdownEmbed
code={"## 一般\n- **ログイン時に開く**\n  - Macのログイン時にMocolammaを開くようにします。このオプションはmacOS版にのみ表示されます。\n- **APIタイムアウト**\n  - APIの最後の応答からタイムアウトエラーにするまでの時間を設定します。モデルの読み込みに長い時間がかかる場合や、ストリームレスポンスをオフにしている場合は時間を長くするか無制限にすることをおすすめします。\n  - デフォルト: `5分`\n\n> **情報**\\\n> モデルのダウンロード時にはAPIタイムアウト設定の影響を受けず、モデルのダウンロードに関するタイムアウト時間は常に無制限になります。\n\n## モデル\n- **モデルリストの並び順を使用**\n  - チャットと画像生成画面のモデル選択において、モデルリストで指定された並び順を使用します。\n- **チャットモデルの並び順**\n  - 「モデルリストの並び順を使用」がオフの場合に、チャット画面のモデル選択における並び順の基準（名前、変更日、サイズなど）と、昇順／降順を設定します。\n- **画像生成モデルの並び順**\n  - 「モデルリストの並び順を使用」がオフの場合に、画像生成画面のモデル選択における並び順の基準と、昇順／降順を設定します。\n\n## 権限\n- **ローカルネットワークアクセス**\n  - Mocolammaにローカルネットワークアクセスの許可が与えられているかどうかを確認します。許可が与えられていないとネットワーク上のOllamaサーバーに接続することはできません。\n\n## App Store機能（App Store版のみ）\n- **レビューを求めないようにする**\n  - 定期的にApp Storeへのレビューを求める画面が表示されないように設定します。\n  - レビューリクエストについて詳しくは、「[**App Storeレビューリクエストについて**](/jp/docs/app-store-review-requests)」ドキュメントをご覧ください。"}
